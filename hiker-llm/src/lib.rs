//! Shared generative-LLM client (status: crawler-shared-llm).
//!
//! Extracted from `core::llm` so `hiker-core` and `hiker-crawler` share ONE
//! client. Wraps the [`llm`](https://crates.io/crates/llm) crate (graniet/llm)
//! so every backend (Anthropic, OpenAI, Ollama, Google, Groq, Mistral,
//! DeepSeek, OpenRouter, ...) reaches callers through one narrow trait. Module
...
//! swapping the multi-provider layer or adding a fallback is a one-crate
//! change.
//!
...
//! transparency live in those consumers — `hiker-llm` is just provider
//! access. Embeddings are a separate concern (`core::embed`); they share the
//! same crate dep but a different trait boundary. The config-bridge from
//! core's `[llm]` TOML section lives in `core::llm` (it references core config
//! types), not here. See `docs/llm.md` and `docs/hiker-crawler.md`
...
/// builder reads it at construction time. The translation from core's
/// `[llm]` TOML section into this struct lives in `core::llm`.
#[derive(Debug, Clone, Serialize, Deserialize)]
