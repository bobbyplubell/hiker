...
- **LLM rewrite to markdown** — most flexible, most expensive, lossy in subtle ways. Lands as the first action in `editor.md`'s `note-mutations-menu`; routes through `core::llm` direct per `llm.md` (single-shot deterministic prompt, output to `.hiker/derived/<rel-path>.md`, source file never touched). Opt-in user action, not an ingest default.
