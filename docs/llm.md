...
- **Sessions persist as markdown notes in the vault.** Each session lives at `vault/.hiker/sessions/<YYYY-MM-DD>-<session-id>.md` (mirrors the path shape of `.hiker/agent-log/` and `.hiker/trash/`). The file is appended to on every `TurnFinished` so a crash mid-session loses at most the in-flight turn. Format: YAML frontmatter (session id, created-at, model, provider, turn count) followed by alternating `## User` / `## Agent` sections in the order the turns landed; tool calls render as fenced blocks under the agent section that issued them. Reading the file is enough to reconstruct the session — the markdown is the source of truth, the in-memory `SessionState` is a working cache rebuilt from it. [chat-session-markdown-store]
...
- **Expand to center pane.** A toggle button in the chat region's header flips chat from "bottom of the discovery panel" into "fills the editor pane" — chat takes over the center column, the prior pane content (editor buffer, home view, settings) is hidden but kept in memory. Toggle a second time and the prior pane content is restored, chat returns to its docked-bottom shape. Mechanically this is a new sub-mode of the editor pane state alongside editor / vault-home-overview / vault-home-detail / settings; the chat state just renders the existing chat surface at the center pane's full size. The discovery panel's other sections (search results, related notes) stay visible in the right column while chat is expanded — only the editor center is replaced. The default split (`chat-panel-default-height`) is unaffected by expand mode and is restored on collapse. Same toggle button shape as the View / Mutations / panel-toggle buttons in the editor toolbar; reserves keybind id `chat.toggle-expand` in `keybind-registry`. [chat-panel-expand-to-editor]
