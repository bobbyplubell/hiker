...
- **Creating a new chat session reveals the chat panel.** The "New chat session" action (toolbar / palette / keybind) creates the session and forces the chat panel visible — un-collapsing it if the user had hidden it. The session itself is useless if the panel that renders it is hidden; "create" is the user's implicit "show me this now" verb. Mirrors the create-then-reveal posture every other creation verb uses (new note opens the buffer, new tree expands the tree mode). [chat-new-session-reveals-panel]
...
A discriminated-union enum emitted on the event chat events. Every event carries `turn_id` (one per user message) and most carry `step_id` (one per LLM call within a turn — increments on each tool-loop iteration). [agent-event-stream-shape]
...
- **MCP server stays available.** Hiker becomes a pure local notes app on the inside while still exposing the vault to external agents (Claude Code, Codex) that the user runs out-of-process and bills outside hiker. This is deliberate: disabling AI *inside* hiker doesn't mean the user is hostile to AI generally; it means hiker shouldn't host it.
- **Deterministic pipelines stay live.** Indexing, embedding, search, clustering build (the non-summarization parts), wikilink resolution, op-log, sync — none of these are AI in the sense the gate targets. They keep running.

Sticky across vaults: `[llm].enabled` defaults to user-scope so a user who turns AI off once doesn't have to re-toggle for every new vault. Per-vault override works via the section's scope toggle.
