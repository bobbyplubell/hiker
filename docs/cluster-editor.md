...
- **Click a leaf** — single left-click both selects the leaf and (when the preview is enabled in the view menu) pins the note into the in-canvas note preview card; **double-click opens the note as a preview tab** in the editor pane (`{ preview: true }`, matching sidebar / activity behavior — the full tab is reachable from the preview's own pin affordance). The in-canvas overlay stays hidden until a leaf is pinned (no empty "click a note" hint card occluding the canvas), and a click on the canvas background clears both the selection and the pinned preview. Clusters route to the right-click popover instead. Nav-stack discipline unchanged. [cluster-editor-graph-view-leaf-click-opens-note]
