...
status:: planned
note:: future ingestion source — `.drawio` (and `.drawio.svg` / `.drawio.png`) diagrams as a source type; structural skeleton (nodes, edges, labels) extracted into a derived hiker note for search/related-notes coverage; diagram file kept as canonical artifact. Anchor: `ideas.md` "Source ingestion"
...
status:: planned
note:: companion browser extension with two primary buttons — "Save to Hiker" (one-click capture into `inbox/`, an external producer feeding the import path, `import.md`) and "Save to Hiker and append to active trail" (same capture, plus append as waypoint to the active trail, greyed when no active trail). Talks to the running app over the existing localhost MCP server; no new transport. Phone share-sheet / Android intent capture is the sibling future on mobile. Anchor: `ideas.md` "Capture surfaces"
...
- **High-fidelity WARC capture** — an external producer records the real HTTP request/response set of a visit as a `.warc`, more faithful than a single-file HTML snapshot. Hiker views it through the warc viewer ([[spec:import-warc-viewer]]), with the markdown shadow as the search layer. [servo-warc-capture]
status:: planned
note:: high-fidelity `.warc` capture by an external producer (more faithful than a single-file HTML snapshot); hiker views it via the warc viewer ([[spec:import-warc-viewer]]), md shadow stays the search layer. Anchor: `ideas.md` "Viewers"

## Editor

- **Split view** — shift-clicking a top-strip tab splits the center pane and renders that tab beside the current one. Tile orientation user-selectable. Splits compose with tab kinds (buffer + agent, buffer + graph, two buffers). Each split holds its own active tab and scroll/selection state; closing a split's last tab collapses it. Open splits ride the autosave tab-state snapshot for workspace restore. Lands after [[spec:tab-kinds]].
- **Special-character visualization** — Notepad++-style toggle rendering non-printable control characters (NULL `0x00`, BS `0x08`, ESC `0x1B`, DEL `0x7F`, C1 controls `0x80`–`0x9F`, BOM) as distinct inline glyphs. Pairs with [[spec:view-show-whitespace-toggle]] (whitespace) by covering the *non*-whitespace controls. Rides a decoration provider emitting `Replace { display }` widgets over visible ranges — same shape as the live-preview markdown decorations. View-menu toggle, default off, persisted per-vault. Optional sub-toggle distinguishes line-ending styles (CRLF / LF / CR).
- **Hex view mode** — new `kind: "hex"` tab (per [[spec:tab-kinds]]) over a file path; renders the standard hex-editor layout (offset / hex bytes / ASCII columns) with hover-pairing between the hex and ASCII halves. Read-only in v1. Opened via filetree right-click "Open as hex" or a View-menu "View as hex" entry. Useful for binary-adjacent files and suspect extracted text. Renders via an egui painter + custom decoration set, lazy on first hex tab.
- **Linked / targeting tabs** — now specced and built for graph + canvas: see `editor.md`'s "Linked tabs (drive / follow)" section ([[spec:tab-link-model]] / [[spec:tab-link-drive]] / [[spec:tab-link-follow]] / [[spec:tab-link-control]] / [[spec:tab-link-persist]]). Remaining reach (a Related-notes tab choosing its own source/target tab; extending the same wiring to the cluster vector-space visualization, [[spec:cluster-vector-viz]]) lands with those features. [tabs-linked-targeting]

## Rendered-diagram node navigation

- **Click a diagram node → its source, with footprint highlight** — clicking a node in a rendered mermaid diagram selects the first occurrence of that node's id in the fenced source. One action, two payoffs: it drops the caret into the node for editing, and — because the existing occurrence-highlight fires on any non-empty selection (`editor-view/src/highlight.rs`) — every other place that node appears lights up. That turns "a node is referenced many times" from a problem into the feature: you see the node's whole footprint. Extends `editor-widgets.md` ([[spec:widget-block-click-to-edit]], [[spec:widget-table-cell-edit]], [[spec:editor-widget-click-regions]]); the whole-diagram body click (caret at source top) stays the fallback. The app-side logic (emit click-zones for *all* regions a diagram reports, select the first occurrence of `region.id`) is **diagram-type-agnostic** — built once, it works for whatever types report hit regions, and new types light up with zero app change.
...
- **Session capability partitions** — partition agent sessions so no single session ever holds all three prompt-injection "lethal trifecta" legs at once: exposure to untrusted content, vault-write, and arbitrary outbound network. A web-facing session can fetch + read but not write the vault; a vault-editing session can write but not reach untrusted hosts. This is the in-process generalization of the import trust boundary ([[spec:import-external-producer-boundary]]) — capability isolation as a structural property of a session rather than a UI mode the model can be talked out of. Broad — touches `mcp.md`, the chat surface, and `task-queue.md`; wants its own spec when picked up. [agent-session-partitions]
status:: planned
note:: enforced partitions so no agent session holds all three prompt-injection trifecta legs at once (untrusted content + vault-write + arbitrary-net); in-process generalization of the import trust boundary ([[spec:import-external-producer-boundary]]). Touches `mcp.md`/chat/`task-queue.md`; own spec when picked up. Anchor: `ideas.md` "Agent session capabilities"
...
status:: planned
note:: parallel chunk-level clustering: tightly-clustered chunks across notes surface as "thread" hints (not auto-trails). Anchor: `ideas.md` "Clustering"
...
status:: planned
note:: parallel chunk-level clustering: a note whose chunks scatter across clusters is a split candidate. Anchor: `ideas.md` "Clustering"
...
- **Sticky manual overrides across re-runs.** Make a user's manual reassignments (move note → cluster) and per-note pins survive a rebuild / re-triage, so curation isn't lost when an Evergreen tree re-runs against fresh data. The cluster editor already does the reassignment ([[spec:cluster-editor-node-operations]]); the new part is persisting those decisions as build/triage constraints the next pass respects. [cluster-sticky-overrides]

## RAG chat

- **RAG chat over the vault** — *external, not in-app.* Hiker has no in-app chat after the core rework. The realized form is an external agent (Claude Code, Goose, …) connected to hiker's MCP server (`mcp.md`), which already exposes search / related / read tools; the agent does its own RAG over those tools. Re-adding an in-app interactive surface (basic loop or ACP client) is a re-openable decision, not a current direction.

## Multi-device sync

*Deferred / re-openable.* The always-on libp2p sync engine and the integrated git push/pull driver were **removed** in the core rework — there is no sync engine today. A vault is plain files, so multi-device today is whatever a third-party file sync (Syncthing, iCloud/Dropbox) or **optional user-driven git** (`git.md`) does.

- **A real sync engine** (encrypted peer transport, enrollment, automatic merge) is a re-openable future idea, not a shipped subsystem. If pursued it would sit *over* the plain-file model (`op-log.md`), not as the always-on substrate the prior design made it. A live concurrent-editing mode (streaming the unsaved `working` overlay between devices, Yjs-style) would be a further layer on top — the layered text model (`accepted`/`working`/`pending`) plus the editor's change-set stream ([[spec:editor-transactions-out]]) are the right foundation, but the transport + convergence engine are the work, and neither exists.
...
- **Live call-flow in the flow dots** — drive the code graph's animated edge-flow dots (`graph-view.md` flow feature) from *actual runtime function calls* instead of the uniform decorative animation: as a traced program runs, edges that are actually being called light up / pulse, with dot density/speed proportional to observed call rate. A real-time call-graph visualization layered on the static call graph.

  Why deferred (likely too complex for v1, but a compelling future feature):
  - **ptrace is the wrong primitive** — breakpoint/single-step tracing is a 10–1000× slowdown and very low-level. The right tools are **eBPF + uprobes** (Linux: attach to function-entry addresses of a running binary, no recompile, symbol-resolved — but per-OS: mac = dtrace pid provider), `-finstrument-functions` (Clang/GCC/Rust-via-LLVM `__cyg_profile_func_enter/exit`, needs a recompile), or a **sampling profiler** (`perf`, async-profiler, py-spy) — sampling is arguably the best fit since flow dots want a per-edge *rate*, not a pulse per call. Managed runtimes have built-in hooks (Python `sys.setprofile`, V8/JFR/`runtime/trace`).
  - **The hard parts are not the tracing.** (1) **Identity mapping**: runtime symbol (mangled name / `file:line` / address) → the SCIP symbol id that is the graph node — the same identity-matching problem the indexer already wrestles with. (2) **Scale**: don't trace the whole program — scope to the visible neighborhood (eBPF can attach/detach as the viewport changes) or use sampling for rates.
  - **Engine side is small and reusable.** Flow is engine-owned and already per-edge (dots ride each `Source::edges()` pair) but uses *uniform* params. Adding a **per-edge flow weight/event channel** (a per-instance attribute the flow shader reads) makes it data-driven for any `Source` — a bounded extension, not a rewrite.
  - **De-risked path**: define a tiny trace-event schema (`{caller_symbol, callee_symbol, count|timestamp}`) and ingest an **offline** trace first (a `perf script` dump / Chrome `trace.json` / custom log) to prove the viz + identity mapping + the per-edge channel with zero tracing machinery; add a **live eBPF backend** as a swappable adapter afterward. Models naturally as a *dynamic overlay* feeding the static graph — same spirit as the spec-engine `DerivedNodeSource` philosophy. [code-view-live-callflow]
...
- **Cluster vector visualization** — projection of the existing note-level embeddings ([[spec:cluster-note-embeddings]]) rendered in a canvas tab, colored by cluster membership from the active cluster tree, label-on-hover, click-to-open. Compatible with [[tabs-linked-targeting]] — a viz tab can highlight the active editor's note and/or drive a target editor tab. [cluster-vector-viz]
...
- **Copy as rich text** — selection → HTML (and optionally wiki markup) on the clipboard, so pasted content keeps formatting in external tools. Conversion runs over the same markdown parse the renderer uses. [editor-copy-rich-text]
...
- **Terminal tab** — a `kind: "terminal"` tab (per [[spec:tab-kinds]]) hosting a real PTY (`portable-pty` or similar), so a working shell sits alongside notes without leaving the app. Per-tab cwd defaults to the vault root. v1: one tab = one shell, no multiplexing. [terminal-tab]
