...
- **Editor-pane "Add to trail" affordance** — when a regular note is open in the editor and a trail is active, a small pill in the editor toolbar (next to the existing `#mode-controls` slot) reads "Add to trail: <trail-name>" and clicks-through to the same op. Hidden when no trail is active, or when the open buffer is itself a trail-doc / waypoint-note / read-only preview. The trail name in the pill doubles as the always-visible "which trail is active" indicator the spec otherwise lacks outside Trails mode. [trail-add-to-active-from-editor-verb]
