...
- **Split-add control.** The `+`-with-dropdown split-button (`split-add-button` in `files.md`) is ghost too: at rest just the `+` and caret glyphs; on hover the engaged half fills, a seam divides the two halves, and a border wraps the control. [style-ghost-button]
