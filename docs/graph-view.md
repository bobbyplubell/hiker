...
- **Per-node label LOD + scale.** Each `NodeDescriptor` carries a `label_min_zoom` (the gate, magnification-aware: effective zoom is `label_zoom · mag`, so central nodes under a lens reveal finer detail than the rim) and a `label_scale` (per-node font-size multiplier). Identical label texts are de-duplicated per frame (the most prominent instance wins), so always-on high-level labels don't pile up. The code graph maps these by entity kind — modules always-on and larger, then types → functions → methods → leaf consts/fields revealed progressively as you zoom in. [graph-label-lod]
status:: done
touches:: [[code:hiker/graph_view/edges]], [[code:hiker/panels/code_graph]]
note:: per-node label gate (magnification-aware `label_zoom·mag`) + per-node font scale; identical labels de-duped per frame. Code graph maps by kind: modules always-on + larger, then types→functions→methods→leaves revealed on zoom-in · evidence: `widgets/graph-view/src/graph_view/edges.rs` (`draw_nodes`), `source.rs` (`NodeDescriptor.label_min_zoom`/`label_scale`), `app/src/panels/code_graph.rs` (`label_*_for`)
