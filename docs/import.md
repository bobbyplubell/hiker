...
- **Versioning rides the op-log.** Re-importing a changed source lands an
  `extractor`-authored op on the note's accepted state, so history / diff / restore
  come from the existing op-log surfaces (`op-log.md`). Binary artifacts (the
  original, an archive) are retained per a user-set cascade and stay device-local. [import-versioning-oplog]
