    ...
    pub fn spawn(initial: Vec<Vec2>, edges: Vec<(u32, u32)>, params: LayoutParams) -> Self {
        ...
        let handle = thread::Builder::new()
            ...
            .spawn(move || {
                ...
                // (mean step + mean swing under `convergence_eps` for
                // `convergence_streak` iters in a row) or until
                // `stop` flips on Drop. Inlined from the former
                // `run_layout` helper so the worker is self-contained.
                let mut pos = initial;
                ...
                for iter in 0..params.max_iters {
                    ...
                    if mean_step < params.convergence_eps
                        && mean_swing < params.convergence_eps
                    {
                        converged_streak += 1;
