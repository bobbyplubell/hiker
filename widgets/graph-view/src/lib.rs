...
//!
//! A caller supplies a [`Source`] adapting its own data into per-frame
//! [`NodeDescriptor`]s plus edge/layout-tree topology, and two host
//! callbacks: the preview-card painter passed to [`State::ui`] and the eye
//! [`egui::Image`] passed to [`State::view_options_menu`]. That keeps the
//! engine free of the app's icon registry and preview-card painter.
