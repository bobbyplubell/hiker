        ...
        match &ops[i] {
            ...
            Op::Delete(_) | Op::Insert(_) => {
                ...
                edits.push(InputEdit {
                    ...
                    new_end_position: shift_point(
                        byte_to_point(before, start_byte),
                        insert_len,
                    ),
                });
                in_pos = old_end_byte;
...
/// Cheap approximation: assume the insertion is a single-line addition.
/// Multi-line inserts will still parse correctly (only byte offsets are
/// authoritative); the point hint is just slightly off.
fn shift_point(p: Point, bytes: usize) -> Point {
    Point { row: p.row, column: p.column + bytes }
}
