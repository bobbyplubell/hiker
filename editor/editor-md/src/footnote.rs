...
pub fn footnote_decorations(
    ...
    collect_definitions(state, &text, fg, def_bg, viewport.as_ref(), &mut entries);
    collect_inline_refs(state, &text, fg, viewport.as_ref(), &mut entries);
...
fn collect_definitions(
    ...
    for line in line_range {
        ...
        let Some(prefix_len) = parse_def_prefix(line_text) else {
            continue;
        };
        entries.push((
            ls..le,
    ...
    while i + 3 < scan_end {
        if bytes[i] == b'[' && bytes[i + 1] == b'^' {
            ...
            if i == ls {
                ...
                let raw = &text[ls..le];
                let line_text = raw.strip_suffix('\n').unwrap_or(raw);
                if parse_def_prefix(line_text).is_some() {
                    i += 1;
                    continue;
                }
            }
