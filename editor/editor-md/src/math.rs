...
pub fn math_decorations(
    ...
    let block_ranges = collect_block_math(state, &text, bg, viewport.as_ref(), &mut entries);
    collect_inline_math(&text, &block_ranges, fg, bg, viewport.as_ref(), &mut entries);
