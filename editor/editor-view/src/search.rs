...
use editor_core::{
    ChangeSet, Color, Decoration, DecorationSet, EditType, EditorState, MarkStyle, RangeSet,
    Transaction,
};
...
pub fn run_search(state: &EditorState, query: &str, flags: SearchFlags) -> Vec<Range<usize>> {
    ...
    let raw: Vec<Range<usize>> = if flags.regex {
        regex_matches(haystack, query, flags.case_sensitive)
    } else {
        substring_matches(haystack, query, flags.case_sensitive)
    };
    ...
    if flags.whole_word {
        ...
        out.retain(|r| is_whole_word(bytes, r.clone()));
    }
    out
}

fn substring_matches(haystack: &str, needle: &str, case_sensitive: bool) -> Vec<Range<usize>> {
    if case_sensitive {
        haystack
        ...
        let lower_n: String = needle.to_lowercase();
        if lower_n.is_empty() {
            return Vec::new();
        }
        // Build map: lowercased-byte-offset -> original-byte-offset.
...
fn regex_matches(haystack: &str, pattern: &str, case_sensitive: bool) -> Vec<Range<usize>> {
    let final_pat = if case_sensitive {
        pattern.to_string()
    } else {
        format!("(?i){pattern}")
    };
    let re = match regex::Regex::new(&final_pat) {
        Ok(r) => r,
...
fn is_whole_word(bytes: &[u8], r: Range<usize>) -> bool {
