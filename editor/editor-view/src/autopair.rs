...
use editor_core::{
    ChangeSet, EditType, EditorState, SelRange, Selection, Transaction,
};
...
/// Look up a pair by opener char.
fn find_pair(c: char) -> Option<&'static AutoPair> {
    DEFAULT_PAIRS.iter().find(|p| p.open == c)
}
...
pub fn autopair_transform(state: &EditorState, inserted: &str) -> Option<Transaction> {
    ...
    let mut positions: Vec<usize> = ranges.iter().map(|r| r.start()).collect();
