...
//!
//! Cluster trees are per-tree `.md` files under `vault/.hiker/trees/`
//! (`trees-md-store`); the `Db` handle owns the op-log + vault references
//! used to read and rewrite a tree's frontmatter. Frontmatter
//! (de)serialization lives in `super::store`.
...
/// Owner of the per-tree `.md` files under `vault/.hiker/trees/`
/// (`trees-md-store`). Reads and rewrites a tree's frontmatter through the
