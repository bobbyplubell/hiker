//! Working-layer + scenario tests for the layered doc: the editable buffer
...
// `every_accepted_version_is_loadable_after_edit_and_save` exercised the
// deleted `.ops` history engine (`doc_history` + `materialize_at`); local
// version history is now plain-file snapshots (`core::snapshot`), tested there.
...
fn ensure_on_disk_restores_a_file_that_vanished_with_no_pending_edit() {
    ...
    // from disk out-of-band, then the user presses Ctrl+S. The accepted content
...
fn assert_accepted_and_disk(log: &LayeredDoc, dir: &std::path::Path, doc_id: &str, expected: &str) {
...
// The cross-device sync tests (`bug_sync_commit_working_races_remote_apply`,
// `commit_working_preserves_peer_edit_during_race`, `bug_sync_working_mirror_cross_lineage_apply`)
// exercised the deleted sync substrate (`apply_remote_update`/`export_state`/
// `adopt_lineage`) and went with it (`hiker-core-rework-plan.md` WS1). The
// commit-path three-way working-mirror merge they leaned on stays and is
// covered by `external_edit_preserves_working_edits`.
