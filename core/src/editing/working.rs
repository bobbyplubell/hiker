...
//! a second `impl LayeredDoc` block kept here so `mod.rs` stays within its
//! file-length budget; they share the same private lock / `ensure_loaded`
//! machinery defined alongside `LayeredDoc`. `commit_working` (the Save bridge into
//! the commit path) stays in `mod.rs` next to `commit_text_edit`.

use super::error::Error;
use super::overlay;
use super::{DocContent, LayeredDoc};
...
impl LayeredDoc {
    ...
    /// the whole model each edit), so it hands the layered doc the full text; this
    ...
    /// ([`apply_remote_update`](super::LayeredDoc::apply_remote_update) keeps
    ...
    /// status: op-log-working-layer, canvas-layered-binding
