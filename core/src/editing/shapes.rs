//! Typed op shapes and the author vocabulary a pending edit carries.
//!
//! A pending edit is a text change (anchored find-replace, whole-doc content,
//! or rename) recovered from `metadata`; the layered model layers a logical
//! [`OpKind`] over it so review surfaces and agent introspection have a typed
//! handle. The kind lives on the [`PendingOp`] while pending; on accept it is
//! applied to `accepted` (the canonical `.md`) and the pending entry is dropped
//! — there is no durable history frame. [`Author`] tags who proposed the edit,
//! with prefix-class wildcard query support (`agent:%`).
//
...
/// The logical shape of one op. One logical op = one pending edit (applied to
/// `accepted` on accept). `SetFrontmatter` is a *logical label*
...
impl OpKind {
    /// Stable wire string for the op kind (used by review surfaces and the
    /// `.pending` serialization). The `Rename` variant's `from` is carried
    /// separately, so the kind string stays a flat enum tag.
...
/// Author of an op. Carried on each [`PendingOp`] to tag who proposed the edit.
/// The wire form is `class[:identifier]`;
/// [`Author::as_wire`] / [`Author::parse`] round-trip it. The class prefix
/// drives prefix-class wildcard matching (`agent:%`).
///
/// status: op-log-author-classes
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum Author {
